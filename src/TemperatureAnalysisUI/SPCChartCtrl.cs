...
{
    public partial class SPCChartCtrl : UserControl
    {
        private string m_sPicName = "";
        private ContextMenuStrip docmenu = new ContextMenuStrip();
        private ToolStripMenuItem saveimg = new ToolStripMenuItem();

        public DataTable dt = new DataTable();

        private Font f12 = new Font("arial", 12, FontStyle.Bold, GraphicsUnit.Pixel);
        private Pen B1pen = new Pen(Color.Black, 1);
        private Pen B2pen = new Pen(Color.Black, 2);
        private double XDoublkeBAR = 0;
        private double RBAR = 0;
        private double XBARUCL = 0;
        private double XBARLCL = 0;

        private double RANGEUCL = 0;
        private double RANGELCL = 0;
        private double[] intMeanArrayVals;
        private double[] intRangeArrayVals;
        private double[] intSTDEVArrayVals;
        private double[] intXBARArrayVals;

        private int First_chartDatarectHeight = 80;
        private Font f10 = new Font("arial", 10, FontStyle.Bold, GraphicsUnit.Pixel);
        private LinearGradientBrush a2 = new LinearGradientBrush(new RectangleF(0, 0, 100, 19), Color.DarkGreen, Color.Green, LinearGradientMode.Horizontal);
        private LinearGradientBrush a3 = new LinearGradientBrush(new RectangleF(0, 0, 100, 19), Color.DarkRed, Color.Red, LinearGradientMode.Horizontal);
        private LinearGradientBrush a1 = new LinearGradientBrush(new RectangleF(0, 0, 100, 19), Color.Blue, Color.DarkBlue, LinearGradientMode.Horizontal);
        ...
        public double CokPpkValue
        {
            get
            {
                return CpkPpKAcceptanceValue;
            }
            set
            {
                CpkPpKAcceptanceValue = value;
            }
        }
        ...
        public void PicBox_Paint(object? sender, PaintEventArgs e)
        {
            ...
            for (int iCol = 1; iCol <= dt.Columns.Count - 1; iCol++)
            {
                ...
                {
                    Sumresult = Sumresult + Convert.ToDouble(dt.Rows[iRow][iCol].ToString());
                    double accountLevel = Convert.ToDouble(dt.Rows[iRow][iCol].ToString());
                    minRangeValue = Math.Min(minRangeValue, accountLevel);
                    maxRangeValue = Math.Max(maxRangeValue, accountLevel);
                    locStdevarr[iRow] = Convert.ToDouble(dt.Rows[iRow][iCol].ToString());
                }
